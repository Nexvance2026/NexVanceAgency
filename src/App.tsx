  ...
  return (
    <div className="min-h-screen bg-[#030712] text-white font-sans antialiased selection:bg-blue-500/30 selection:text-blue-200 overflow-x-hidden">
      ...
      <main className="relative z-10 pt-28 max-w-7xl mx-auto w-full">
        ...
        {activeTab === 'overview' && (
          <div className="animate-fadeIn px-6 md:px-[8%]">
            ...
            <div className="text-center max-w-4xl mx-auto mb-16">
              ...
              {/* Action Buttons to go to the forms */}
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                ...
                >
                  <span>Work with NexVance</span>
                  <span className="transform group-hover:translate-x-1 transition-transform">→</span>
                </button>
                <button 
                  onClick={() => setActiveTab('creators')}
                  className="w-full sm:w-auto bg-neutral-900 hover:bg-neutral-800 text-white border border-white/10 font-semibold px-8 py-4 rounded-xl transition-all duration-300 flex items-center justify-center space-x-2 group"
                >
            ...
            {/* BRAND <-> CREATOR HIGH TECH MIDDLEMAN PIPELINE GRAPH */}
                ...
                <div className="relative z-10 border-t border-white/5 pt-4 bg-gradient-to-t from-black via-black/90 to-transparent">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3">
                    ...
                    <div className="flex space-x-1.5">
                      ...
                      <span className="bg-neutral-900 border border-white/10 px-2.5 py-1 rounded text-[9px] font-mono text-white font-semibold">Zero Drag Execution</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-neutral-900/30 border border-white/5 p-6 rounded-2xl backdrop-blur-md mb-28 max-w-4xl mx-auto">
              ...
              <div className="text-center p-2 border-r border-white/5 last:border-0">
                ...
                <div className="text-[10px] uppercase tracking-wider text-neutral-500 mt-1">Combined Reach</div>
              </div>
        ...
        {/* ==================== 2. FOR TECH BRANDS PAGE ==================== */}
        {activeTab === 'brands' && (
          <div className="animate-fadeIn px-6 md:px-[8%]">
            <div className="max-w-4xl mx-auto bg-neutral-900/40 border border-white/5 p-8 rounded-2xl backdrop-blur-md mb-24 grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
              <div className="md:col-span-5">
                ...
                <h3 className="text-3xl font-bold tracking-tight mb-4">Launch campaigns that convert.</h3>
                <p className="text-neutral-400 text-xs leading-relaxed mb-6">Partner with NexVance to manage briefs, content QA, talent escrow, and analytical tracking infrastructure for high-value tech partnerships.</p>
              </div>
              ...
              <form action="https://formspree.io/f/xlgkywng" method="POST" className="md:col-span-7 space-y-4 w-full">
                ...
                <div>
                  ...
                  <textarea name="message" rows={4} placeholder="Tell us about your product launch plan..." className="w-full bg-black/40 border border-white/10 rounded-lg p-3 text-xs text-white focus:border-blue-500 outline-none transition-colors resize-none" />
                </div>
                <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 py-3 rounded-xl text-xs font-semibold tracking-wide transition-all shadow-[0_4px_15px_rgba(37,99,235,0.3)]">Submit Strategy Request</button>
              </form>
            </div>
          </div>
        )}
          ...
          <div className="animate-fadeIn px-6 md:px-[8%]">
            <div className="max-w-4xl mx-auto bg-neutral-900/40 border border-white/5 p-8 rounded-2xl backdrop-blur-md mb-24 grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
              <div className="md:col-span-5">
                ...
                <h3 className="text-3xl font-bold tracking-tight mb-4">Focus on your content.</h3>
                <p className="text-neutral-400 text-xs leading-relaxed mb-6">Join our active roster to get steady inbound deal flow, structured protection, and zero negotiation drag.</p>
              </div>
              ...
              <form action="https://formspree.io/f/xlgkywng" method="POST" className="md:col-span-7 space-y-4 w-full">
                <div>
                  ...
                  <input required type="text" name="channelName" placeholder="e.g., TechReviewer" className="w-full bg-black/40 border border-white/10 rounded-lg p-3 text-xs text-white focus:border-emerald-500 outline-none transition-colors" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-neutral-400 mb-1.5">Contact email</label>
                  <input required type="email" name="email" placeholder="you@domain.com" className="w-full bg-black/40 border border-white/10 rounded-lg p-3 text-xs text-white focus:border-emerald-500 outline-none transition-colors" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-neutral-400 mb-1.5">Primary Platform & Stats</label>
                  <textarea name="message" rows={3} placeholder="Subscribers, audience niche, links..." className="w-full bg-black/40 border border-white/10 rounded-lg p-3 text-xs text-white focus:border-emerald-500 outline-none transition-colors resize-none" />
                </div>
                <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 py-3 rounded-xl text-xs font-semibold tracking-wide transition-all shadow-[0_4px_15px_rgba(16,185,129,0.3)]">Apply to Roster</button>
              </form>
            </div>
          </div>
        )}
